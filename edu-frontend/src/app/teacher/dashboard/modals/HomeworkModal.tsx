// src/app/teacher/dashboard/modals/HomeworkModal.tsx
"use client";
...
export default function HomeworkModal({
  ...
  const [attachments_url, setAttachmentsUrl] = useState<string>(initial?.attachments_url ?? "");
  const [is_active, setIsActive] = useState<boolean>(Number(initial?.is_active ?? 1) === 1);

  // Reset safely when opening different record
  useEffect(() => {
    if (!open) return;
    ...
    setAttachmentsUrl(initial?.attachments_url ?? "");
  ...
  const submit = async () => {
    ...
    await onSave({
      ...
      attachments_url: attachments_url.trim() ? attachments_url.trim() : null,
      ...
      <div className={cx("w-full max-w-2xl overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl", ar && "text-right")} dir={ar ? "rtl" : "ltr"}>
        ...
        <div className="max-h-[75vh] overflow-auto p-4">
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <label className="sm:col-span-2">
              <div className="mb-1 text-xs font-semibold text-slate-700">attachments_url</div>
              <input value={attachments_url} onChange={(e) => setAttachmentsUrl(e.target.value)} className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" />
            </label>
          </div>
        </div>
