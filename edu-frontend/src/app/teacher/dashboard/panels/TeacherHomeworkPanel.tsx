// src/app/teacher/dashboard/panels/TeacherHomeworkPanel.tsx
"use client";

import { Download, Plus, Search } from "lucide-react";
import type { Lang, HomeworkRow } from "../teacherDashboardTypes";

export type TeacherHomeworkPanelProps = {
  lang: Lang;
  q: string;
  onQChange: (v: string) => void;
  rows: HomeworkRow[];
  onCreate: () => void;
  onEdit: (row: HomeworkRow) => void;
  onOpenSubmissions: (row: HomeworkRow) => void | Promise<void>;
  onExportCSV: <T extends Record<string, unknown>>(rows: T[], filename: string) => void;
};

export default function TeacherHomeworkPanel({
  lang,
  q,
  onQChange,
  rows,
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div className="flex flex-col gap-2 border-b border-slate-100 p-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div className="text-sm font-semibold text-slate-900">{ar ? "الواجبات" : "Homework"}</div>
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
          ...
          <button
            type="button"
            onClick={() => onExportCSV(rows as unknown as Record<string, unknown>[], "teacher_homework")}
                ...
                <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                  <div className="min-w-0">
                    ...
                    <div className="mt-1 text-xs text-slate-600">
                      ...
                      {" • "}
                      {ar ? "تاريخ التسليم:" : "Due:"} {r.due_at?.slice(0, 16)}
                      {" • "}
